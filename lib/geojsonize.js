...
function geojsonize_and_upload_blob(country_code, admin_level){
  ...
  var output = temp_storage + 'geojson/' + country_code + '_' + admin_level + '.geojson';
    ...
    if (fs.existsSync(input) ) {
      ...
      ogr.exec(function (er, data) {
        fs.writeFile(output, JSON.stringify(data), function(err) {
          ...
          azure.upload_blob(geojson_container, country_code + '_' + admin_level, temp_storage + 'geojson', 'geojson').then(function(){
            resolve();
          });
        });
      });
